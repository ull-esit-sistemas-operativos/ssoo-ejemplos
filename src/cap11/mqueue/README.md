...
En [`message_queue.hpp`](posix/message_queue.hpp) se puede ver un ejemplo de una clase desarrollada en C++ para utilizar colas de mensajes POSIX.
...
En [`mqueue.cpp`](posix/mqueue.cpp) y [`mqueue-control.cpp`](posix/mqueue-control.cpp) se ilustra cómo se utiliza la clase en [`message_queue.hpp`](posix/message_queue.hpp).
...
El ejemplo es muy sencillo, así que, por el momento, lo único que puede hacer [`mqueue-control.cpp`](posix/mqueue-control.cpp) es pedirle a [`mqueue.cpp`](posix/mqueue.cpp) que termine y para eso utiliza una cola de mensajes.
