...
int main()
{
    ...
    // error. Por ejemplo, si al salir por un error olvidamos eliminar el objeto de memoria
    // compartida, no podríamos volver a ejecutar el servidor hasta borrarla a mano. En C++ es
    // mejor encapsular los recursos en clases y usar su destructor para liberarlos (RAII).
...
int create_control_shm(int* controlfd)
{
    // Como no hay función en el estándar de C ni en el de C++ para crear objetos de memoria
    // compartida, usamos directamente shm_open() de la librería del sistema.
    *controlfd = shm_open( CONTROL_SHM_NAME, O_CREAT | O_RDWR, 0777 );
    if (*controlfd < 0)
    {
        if (errno == EEXIST)
        {
            // Si ya existe el objeto de memoria compartida, no deberíamos usarlo porque habrían
            // varios servidores usando el mismo canal de control y no hemos pensado en eso
            // al diseñarlo.
    ...
    // El objeto de memoria compartida creado tiene tamaño 0. Tenemos que extenderlo para que
    // quepa 'memory_content'.
...
int init_control_memory(int controlfd, struct memory_content** memory_region)
{
    ...
    *memory_region = mmap( NULL, sizeof(struct memory_content), PROT_READ | PROT_WRITE,
        MAP_SHARED, controlfd, 0 );
    ...
    // Necesitamos un mecanismo para sincronizar cliente y servidor. El cliente necesita esperar a
    // que el servidor lea el último comando antes de escribir el siguiente. El servidor debe
    // esperar a que el cliente ponga un comando antes de intentar leer.
    // Para esto el sistema ofrece mecanismos de sincronización, como los semáforos.
    //
    // Incializar los semáforo.
